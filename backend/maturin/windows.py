def os_build(common, versions, project, wheels, build, post):
    ...
    for version in versions:
        for version_string, python, target in (
            (version, '"C:\\Program Files\\Python{}\\python.exe"'.format(version.replace('.', '')), 'x86_64-pc-windows-msvc'),
            (f'{version} (32 bits)', '"C:\\Program Files (x86)\\Python{}-32\\python.exe"'.format(version.replace('.', '')), 'i686-pc-windows-msvc'),
        ):
